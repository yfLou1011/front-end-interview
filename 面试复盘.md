...
- 联动表单: 
  ...
  - 位置状态联动多
  - 表单依赖多，A输入框的状态会根据B输入框的value变化导致 校验规则、显隐状态等变化
  - 组件类型多，有简单的输入框，也有SKU数据这种动态增删的组件，不同的类目下，展示规则还不同。
  - 错误提示、输入数据校验和定位。
- 校验规则主要分为3大类: 
  1. disableRule  控制组件是否展示隐藏，优先级最高
  2. requireRule 必填规则校验
  3. otherRule， 比如**数据值类型**、最小长度、最大长度、最小值等等。**临界值**还要判断 是否包含边界。
  4. 还有单纯的提示规则，比如“请认真填写这个字段，这个地段特别重要”。
*** 
